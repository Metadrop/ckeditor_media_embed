...
class CLICommands {
   ...
   *   FALSE otherwise.
   */
  public function askToOverwritePluginFiles($command) {
    $yes = ($command->input->hasOption('yes'))
      ? $command->input->getOption('yes')
      : FALSE;
   ...
   */
  public function overwritePluginFiles($command, $overwrite) {
  ...
  protected function downloadCKEditorFull($command) {
   ...
   */
  // @codingStandardsIgnoreLine
  protected function installCKEditorPlugin($command, $package_directory, $plugin_name, $overwrite = FALSE) {
    ...
    try {
      ...
      $command->io->success(sprintf($command->getMessage('success-installed-plugin'), $plugin_name));
    }
